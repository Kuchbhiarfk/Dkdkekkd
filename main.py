...
async def op_command(bot: Client, m: Message):
    ...
    print(f"User {m.from_user.id} started /op with batch: {b_name}")  # Debug
...
async def text_handler(bot: Client, m: Message):
    ...
    if user_id not in user_links:
        print(f"User {user_id} sent text without /op: {m.text}")  # Debug
    ...
    if match:
        ...
        print(f"User {user_id} added link: {message_text}")  # Debug
        await m.reply_text(f"Link received: {link}\nSend more links or use /doit to process.")
    else:
        print(f"User {user_id} sent invalid link format: {message_text}")  # Debug
...
async def doit_command(bot: Client, m: Message):
    user_id = m.from_user.id
    print(f"User {user_id} triggered /doit")  # Debug
    try:
        if user_id not in user_links or not user_links[user_id]["links"]:
            print(f"No links found for user {user_id}")  # Debug
        ...
        print(f"Processing {len(links)} links for user {user_id}, batch: {b_name}")  # Debug
        ...
        for index, link_text in enumerate(links, 1):
            print(f"Processing link {index}/{len(links)}: {link_text}")  # Debug
            ...
            try:
                ...
                if not match:
                    print(f"Invalid link format for link {index}: {link_text}")  # Debug
                ...
                try:
                    ...
                    if len(parts) >= 3:
                        ...
                        if len(remaining) >= 3:
                            ...
                            raw_text65 = remaining[1].strip()  # Chapter ID (e.g., KEFJUikgUGh5c2ljcy At IFJldmlzaW9uIChQaHlzaWNzKQ)
                        else:
                ...
                if "acecwply" in url:
                    ...
                    try:
                        print(f"Fetching studystark URL: {url}")  # Debug
                        response = requests.get(url)
                        response.raise_for_status()
                        data = response.json()
                        video_url = data.get("video_url", "")
                        print(f"Studystark video_url: {video_url}")  # Debug
                        if video_url:
                            if video_url.startswith("https://sec"):
                                base_path = video_url.split('?')[0].replace('master.mpd', '')
                                query_params = video_url.split('?')[1] if '?' in video_url else ''
                                ...
                                try:
                                    ...
                                    print(f"Error fetching access token: {str(e)}")  # Debug
                                    url = f"{new_url}&token={raw_text4}"
                            elif video_url.startswith("https://next"):
                                if video_url.endswith("master.mpd"):
                                    url = video_url.replace("master.mpd", f"hls/{raw_text97}/main.m3u8")
                        ...
                        print(f"Error processing studystark URL: {str(e)}")  # Debug
                        url = ""
                ...
                try:
                    ...
                    if "drive" in url:
                        print(f"Downloading Google Drive file: {url}")  # Debug
                        ...
                        if "cwmediabkt99" in url:
                            ...
                            for attempt in range(max_retries):
                                try:
                                    ...
                                    url = url.replace(" ", "%20")
                                    scraper = cloudscraper.create_scraper()
                                    print(f"Attempt {attempt + 1}/{max_retries} to download PDF: {url}")  # Debug
                                    response = scraper.get(url)
                                    if response.status_code == 200:
                                        with open(f'{name}.pdf', 'wb') as file:
                                            file.write(response.content)
                            ...
                            print(f"Downloading PDF with yt-dlp: {url}")  # Debug
                            cmd = f'yt-dlp -o "{name}.pdf" "{url}"'
                            download_cmd = f"{cmd} -R 25 --fragment-retries 25"
                            os.system(download_cmd)
                            copy = await bot.send_document(chat_id=m.chat.id, document=f'{name}.pdf', caption=cc1)
                        ...
                        cmd = f'yt-dlp -x --audio-format {ext} -o "{name}.{ext}" "{url}"'
                        download_cmd = f"{cmd} -R 25 --fragment-retries 25"
                        print(f"Downloading audio with yt-dlp: {url}")  # Debug
                        os.system(download_cmd)
                        ...
                        cmd = f'yt-dlp -o "{name}.{ext}" "{url}"'
                        download_cmd = f"{cmd} -R 25 --fragment-retries 25"
                        print(f"Downloading image with yt-dlp: {url}")  # Debug
                        os.system(download_cmd)
                        ...
                        print(f"Downloading encrypted video: {url}")  # Debug
                        res_file = await helper.download_and_decrypt_video(url, cmd, name, appxkey)
                        filename = res_file
                        await prog.delete(True)
                        await helper.send_vid(bot, m, cc, filename, thumb, name, prog, channel_id, watermark=watermark)
                        ...
                        Show = f"**⚡Dᴏᴡɴʟᴏᴀᴅɪɴɢ Sᴛᴀʀᴛᴇᴅ...⏳**\n🖇️ LNK » {url}\n🎓 Uploaded By » {CREDIT}"
                        prog = await m.reply_text(Show, disable_web_page_preview=True)
                        print(f"Downloading DRM video: {url}")  # Debug
                        res_file = await helper.decrypt_and_merge_video(mpd, keys_string, path, name, raw_text2)
                        filename = res_file
                        await prog.delete(True)
                        await helper.send_vid(bot, m, cc, filename, thumb, name, prog, channel_id, watermark=watermark)
                        ...
                        Show = f"**🚀Dᴏᴡɴʟᴏᴀᴅɪɴɢ Sᴛᴀʀᴛᴇᴅ **\n🔗 𝐋𝐢𝐧𝐤 » {url}\n✦ Uploader {CREDIT}"
                        prog = await m.reply_text(Show, disable_web_page_preview=True)
                        print(f"Downloading video with helper: {url}")  # Debug
                        res_file = await helper.download_video(url, cmd, name)
                        filename = res_file
                        await prog.delete(True)
                        await helper.send_vid(bot, m, cc, filename, thumb, name, prog, channel_id, watermark=watermark)
                    ...
                    print(f"Download error for link {index}: {str(e)}")  # Debug
                ...
                if index < len(links):
                    print(f"Waiting 5 minutes before next link")  # Debug
                    await asyncio.sleep(300)  # 5 minutes = 300 seconds

            except Exception as e:
                print(f"Error processing link {index}: {str(e)}")  # Debug
        ...
        print(f"Finished processing for user {user_id}, batch: {b_name}")  # Debug
        # Clear the user's links after processing
        del user_links[user_id]

    except Exception as e:
        print(f"Critical error in doit_command for user {user_id}: {str(e)}")  # Debug
        await m.reply_text(f"⚠️Critical error occurred: {str(e)}")  
